    ...
    while(1){
        if(recvfrom(client_sockfd, &rcv_pkt, sizeof(rcv_pkt), 0, (struct sockaddr*)&send_sock, (socklen_t*)&len) != -1){
            if(strncmp(send_pkt.data,"cal",3)==0){
                ...
                receive_window-=MSS;
                if((rcv_pkt.Header.ACK-1) != serveracknum_testpktloss && firstgetcommand != 1){
...
void server_video(TCP_pkt p){//send video packet (twice) and  receive ack(file size)
    ...
    while(1){//send video
        if(congestion_window>=THRESHOLD && maxsend_packetnum == congestion_window/MSS){
        ...
        if(need_send_byte!=0){
            ...
            if(loss_rate!=0){
                ...
                cout<<"cwnd = "<<congestion_window<<", rwnd = "<<rcv_pkt.Header.window<<", threshold = "<<THRESHOLD<<endl;
                if(lost==0)//not lost
